...
def clean_players() -> dict[str, pd.DataFrame]:
    ...
    players["team_name"] = players["fpl_team_id"].map(team_by_fpl_id)
    ...
    for f in fixtures:
        ...
        upcoming.append({"gameweek_id": f["event"], "fpl_fixture_id": f["id"],
                         "team_name": team_by_fpl_id[f["team_h"]],
                         "opponent_name": team_by_fpl_id[f["team_a"]],
                         "is_home": True, "difficulty": f["team_h_difficulty"],
                         "finished": bool(f.get("finished"))})
        upcoming.append({"gameweek_id": f["event"], "fpl_fixture_id": f["id"],
                         "team_name": team_by_fpl_id[f["team_a"]],
                         "opponent_name": team_by_fpl_id[f["team_h"]],
    ...
    for element_id in players["fpl_element_id"]:
        ...
        for h in json.loads(path.read_text()).get("history", []):
            rows.append({
                ...
                "opponent_name": team_by_fpl_id.get(h["opponent_team"]),
